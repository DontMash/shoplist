...
function ToastViewport({ className, ...props }: ToastPrimitive.Viewport.Props) {
  return (
    <ToastPrimitive.Viewport
      ...
      className={cn(
        "pointer-events-none fixed inset-x-4 bottom-4 z-[1100] mx-auto w-auto max-w-sm outline-none",
        className
      )}
      {...props}
    />
  ...
  return toasts.map((toastItem) => (
    <Toast key={toastItem.id} toast={toastItem}>
      <ToastContent>
        ...
        <ToastAction />
        <ToastClose render={<Button variant="ghost" size="icon-sm" aria-label="Close toast" />} />
      </ToastContent>
    </Toast>
  ))
}
