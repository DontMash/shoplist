...
**Status: accepted** (the `/api` compatibility-adapter statement below is superseded by ADR-0002)
...
During rollout, the existing `/api` and `/ws` handlers remained as compatibility adapters for already-open or embedded clients; new browser connections use the `/rpc` contract and protocol-version mismatch is terminal with an upgrade-required outcome. Those adapters were removed once the deployment window closed; `/api` is now the OpenAPI transport (see ADR-0002).
